function [r] = Streakline_Aditya(U,V,x1,x2,y1,y2,t1,t2,n)
...
for i = 2:1:l+1
    ...
    k1 = dt*U(X(:,1:i-1),Y(:,1:i-1),t(i-1)*ones(n,1));
    l1 = dt*V(X(:,1:i-1),Y(:,1:i-1),t(i-1)*ones(n,1));
    
    k2 = dt*U(X(:,1:i-1)+k1/2,Y(:,1:i-1)+l1/2,(t(i-1)+dt/2)*ones(n,1));
    l2 = dt*V(X(:,1:i-1)+k1/2,Y(:,1:i-1)+l1/2,(t(i-1)+dt/2)*ones(n,1));
    
    k3 = dt*U(X(:,1:i-1)+k2/2,Y(:,1:i-1)+l2/2,(t(i-1)+dt/2)*ones(n,1));
    l3 = dt*V(X(:,1:i-1)+k2/2,Y(:,1:i-1)+l2/2,(t(i-1)+dt/2)*ones(n,1));
    
    k4 = dt*U(X(:,1:i-1)+k3,Y(:,1:i-1)+l3,(t(i-1)+dt)*ones(n,1));
    l4 = dt*V(X(:,1:i-1)+k3,Y(:,1:i-1)+l3,(t(i-1)+dt)*ones(n,1));
    
    X(:,2:i) = X(:, 1:i-1) + k1/6 + k2/3 + k3/3 + k4/6;
    Y(:,2:i) = Y(:, 1:i-1) + l1/6 + l2/3 + l3/3 + l4/6;
    ...
    xlabel('x')
    ylabel('y')
    title('streaklines at t = ' + string(t(i)))
    drawnow
